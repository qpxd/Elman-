...
mse10=[0,0];
%% 2.读取读取
data=xlsread('光强分布x1.xlsx'); %% Matlab2021版本以上无法使用xlsread函数，可用Load函数替代  
...
output_train =output(n(1:trainNum))';  % 训练集输出 同理只不过对象换成output
input_test =input(n(trainNum+1:trainNum+testNum),:)'; % 测试集输入 同理
output_test =output(n(trainNum+1:trainNum+testNum))'; % 测试集输出 同理
%% 4.数据归一化
...
% Elman预测值和实际值的对比图
figure
plot(output_test,'bo-','linewidth',1.5)%蓝色 圆圈 实线 线宽1.5 实际值
legend('实际值')
hold on
plot(test_simu,'rs-','linewidth',1.5)%红色 放个 实线 线宽1.5 预测值
legend('预测值')%创建图例 即标注
...
xlabel('测试样本'),ylabel('预测误差/0.125m')
title('Elman神经网络测试集的预测误差')
set(gca,'fontsize',12)

figure;
plotregression(output_test,test_simu,['Elman回归图']); %绘制output_test对test_simu的线性回归图 test_simu为点 output_test为线
figure;
ploterrhist(test_simu-output_test,['Elman误差直方图']);%绘制output_test对test_simu的误差直方图 
% Elamn隐藏节点数对RMSE的影响
figure
plot(mse10,'bo-','linewidth',1.5)
xlabel('隐藏节点数'),ylabel('MSE-x/0.125m')
title('隐藏节点数对训练集均方误差的影响')
set(gca,'fontsize',12)
xlim([3,12])
...
% save x.mat;
% save('netx.mat','net');保存data
%save ('timex.mat','timex') %保存执行时间（50）
% 工作区中
% output_test代表测试集
% test_simu代表BP预测值
% error代表误差
